    ...
    CargarConcepto: function () {
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#lstConcepto").append($('<option>', { value: 0, text: 'Seleccione' }));
                if (!response.hasOwnProperty('ErrorMessage')) { // Si la petición no emitió error 

                    $.grep(response, function (oDocumento) {

                        $('select[name="lstConcepto"]').append($('<option>', { value: oDocumento["Id"], text: oDocumento["Nombre"] }));
                    });
                }
            }
    ...
    CargarTipoI: function () {
        $.ajax({
            ...
            success: function (response) {
                ...
                $("#lstTipoI").append($('<option>', { value: 0, text: 'Seleccione' }));
                if (!response.hasOwnProperty('ErrorMessage')) { // Si la petición no emitió error 

                    $.grep(response, function (oDocumento) {

                        $('select[name="lstTipoI"]').append($('<option>', { value: oDocumento["Id"], text: oDocumento["Nombre"] }));
                    });
                }
            }
...
$(function () {
    ...
    $("#lstSucursalCab").change(function () {
        $.ajax({
            ...
            dataType: 'json',
            data: { flag: 2, Id: $("#lstSucursalCab").val() },
            success: function (response) {
                $("#lstAlmacen").empty();
                $("#lstAlmacen").append($('<option>', { value: 0, text: 'Seleccione' }));
                if (!response.hasOwnProperty('ErrorMessage')) { // Si la petición no emitió error 
    ...
    $("#btnGuardar").click(function () {
        ...
        if ($("#lstTipoDoc").val() == 0 || $("#lstTipoPago").val() == 0 || $("#lstConcepto").val() == 0 || $("#hdfId").val() == 0 || $("#lstAlmacen").val() == 0) {
            ...
            $.ajax({
                async: true,
                type: 'post',
                url: General.Utils.ContextPath('Gestion/InstRegistrarOrdenCompra'),
                dataType: 'json',
                ...
                $.grep(response, function (item) {
                    if (item["EstadoDoc"] == "APROBADO") {
                        $tb.find('tbody').append(
                            ...
                            '<td class="text-center">' +
                            '<button class="btn-crud btn btn-success btn-sm evento" title="Aprobar"><i class="fa fa-check"></i> </button>' +
                            '</tr>'
                        );
                    }
                });
...
var ObtenerOC = function (Id) {
    ...
    $.ajax({
        async: true,
        type: 'post',
        url: General.Utils.ContextPath('Gestion/ObtnerDetOC'),
        dataType: 'json',
        beforeSend: General.Utils.StartLoading,
        complete: General.Utils.EndLoading,
        data: senData,
        success: function (response) {
            console.log(response)
            $("#IdOrden").val(response[0].OrdenCompraCab.IdCompra)//tipo pago
            $("#lstTipoPago").val(response[0].OrdenCompraCab.Id)//tipo pago
            $("#dFechaPago").val(response[0].OrdenCompraCab.FechaPago)
            $("#lstMoneda").val(response[0].OrdenCompraCab.Moneda.IdMoneda)
            $("#hdfId").val(response[0].OrdenCompraCab.Proveedor.Id)
            $("#lstTipoI").val(response[0].OrdenCompraCab.Proveedor.Id)
            $("#txtDocCli").val(response[0].OrdenCompraCab.Proveedor.NroDocumento)
            $("#txtNomCli").val(response[0].OrdenCompraCab.Proveedor.Nombre)
            $("#txtDireccionInicio").val(response[0].OrdenCompraCab.Proveedor.Direccion)
            $("#txtNroOC").val(response[0].OrdenCompraCab.Serie)
            console.log($("#hdfId").val())
            $.grep(response, function (oDetalle) {
                Lista.Vars.Detalle.push({
                    ...
                    Categoria: oDetalle.Material.Categoria.Nombre,
                    Marca: oDetalle.Material.Marca.Nombre,
                    Unidad: oDetalle.Material.Unidad.Nombre,
                    Cantidad: oDetalle.Cantidad,
                    Precio: oDetalle.Precio,
                    Importe: (parseFloat(oDetalle.Precio) * parseFloat(oDetalle.Cantidad)).toFixed(2),
                    Almacen: { IdAlmacen: $("#lstAlmacen").val() },
                });
            });
            // console.log(Lista.Vars.Detalle)
