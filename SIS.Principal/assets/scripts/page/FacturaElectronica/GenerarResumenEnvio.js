...
$(function () {
    ...
    $('#btnGenerar').click(function () {
        if (CompPagoEnviar.length === 0) {
            ...
            $.ajax({
                url: General.Utils.ContextPath('FacturaElectronica/ResumnetoEnvio'),
               // beforeSend: General.Utils.StartLoading,
                //complete: General.Utils.EndLoading,
                type: 'post',
                data: {
                    sLista: JSON.stringify(CompPagoEnviar), fecha: $("#fechaInicio").val(), sucursal: $("#lstSucursal").val()
                },
                success: function (response) {
...
function cargarComprobantes() {
    ...
    $.ajax({
        async: false,
        type: 'post',
        url: General.Utils.ContextPath('FacturaElectronica/ListarBoletaResumen'),
        //beforeSend: General.Utils.StartLoading,
        //complete: General.Utils.EndLoading,
        dataType: 'json',
        data: { Comienzo: 0, Medida: 20, Sucursal: iSucursal, FechaEmi: vFechaEmi },
            ...
            if (!response.hasOwnProperty('ErrorMessage')) { // Si la petición no emitió error
                ...
                if (response.length == 0) {
                    ...
                    $.grep(response, function (item) {
                        $tb.find('tbody').append(
                            '<tr data-id="' + item["Id"] + '">' +
                            '<td class="text-center">' + item["serie"] + '-' + item["numero"] + '</td>' +
                            '<td class="text-center">' + item["Documento"]['Nombre'] + '</td>' +
                            '<td class="text-center">' + item["fechaEmision"] + '</td>' +
                            '<td class="text-center">' + formatNumber(item["grabada"]) + '</td>' +
                            '<td class="text-center">' + formatNumber(item["inafecta"]) + '</td>' +
                            '<td class="text-center">' + formatNumber(item["exonerada"]) + '</td>' +
                            '<td class="text-center">' + formatNumber(item["gratuita"]) + '</td>' +
                            '<td class="text-center">' + formatNumber(item["descuento"]) + '</td>' +
                            '<td class="text-center">' + formatNumber(item["igv"]) + '</td>' +
                            '<td class="text-center">' + formatNumber(item["total"]) + '</td>' +
                            '<td class="text-center">' + '<input type="checkbox" class="chkEnviar"></input>' + '</td>' +
                            '</tr>'
                        );
