...
namespace SIS.Data
{
    public class DVenta : DBHelper
    {
        ...
        public string RegistrarVenta(EVenta oDatos, List<EVentaDetalle> Detalle, string Usuario, string tipoSistema,
                                    string rutaComprobantes, string rutaServidor, string claveCertificado)
        {
        ...
        public string RegistrarPost(EVenta oDatos, List<EVentaDetalle> Detalle, string Usuario, string tipoSistema,
                               string rutaComprobantes, string rutaServidor, string claveCertificado)
        {
            using (var Connection = GetConnection(BaseDeDatos))
            {
        ...
        public List<EVenta> ListaVenta(string filtro, int Empresa, int sucursal, string FechaIncio, string FechaFin, int numPag, int allReg, int Cant,string vendedor)
        {
                ...
                try
                {
                    ...
                    using (var Reader = ExecuteReader())
                    {
                        while (Reader.Read())
                        {
                            ...
                            obj.inafecta = float.Parse(Reader["OpeExoneradas"].ToString());
                            obj.exonerada = float.Parse(Reader["OpeInafecta"].ToString());
                ...
                try
                {
                    ...
                    using (var Reader = ExecuteReader())
                    {
                        while (Reader.Read())
                        {
                            ...
                            obj.Venta.inafecta = float.Parse(Reader["OpeExoneradas"].ToString());
                            obj.Venta.exonerada = float.Parse(Reader["OpeInafecta"].ToString());
                            ...
                            obj.material.sCodigo = Reader["sCodMaterial"].ToString();
        ...
        public string VerificarStock(int empresa, int almacen, int idSucursal, int IdProducto, double Cantidad)
        {
            using (var Connection = GetConnection(BaseDeDatos))
            {
                try
        ...
        public List<EVenta> ListaVentaEnvios(string filtro, int Empresa, int sucursal, string FechaIncio, string FechaFin, int numPag, int allReg, int Cant)
        {
            List<EVenta> oDatos = new List<EVenta>();
            using (var Connection = GetConnection(BaseDeDatos))
            {
                ...
                {
                    Connection.Open();
                    SetQuery("FAC_ListaFacBolEnvios");
                    CreateHelper(Connection);
                    AddInParameter("@Filltro", filtro);
                    AddInParameter("@IdEmpresa", Empresa);
                    AddInParameter("@IdSucursal", sucursal);
        ...
        public string EstadoEnviado(int IdVenta, int Empresa, string Motivo, int Flag, string Usuario)
        {
            using (var Connection = GetConnection(BaseDeDatos))
            {
                try
