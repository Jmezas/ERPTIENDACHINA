...
namespace SIS.Business
{
    public class BVenta
    {
                             ...
                             string rutaComprobantes, string rutaServidor, string claveCertificado)
        {
            try
            {
                return Data.RegistrarPost(oDatos, Detalle, Usuario, tipoSistema, rutaComprobantes, rutaServidor, claveCertificado);
            }
            catch (Exception ex)
            {
                return ex.Message;
        ...
        public List<EVenta> ListaVenta(string filtro, int Empresa, int sucursal, string FechaIncio, string FechaFin, int numPag, int allReg, int Cant, string vendedor)
        {
            try
            {
                return Data.ListaVenta(filtro, Empresa, sucursal, FechaIncio, FechaFin, numPag, allReg, Cant,vendedor);
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public string VerificarStock(int empresa, int almacen, int idSucursal, int IdProducto, double Cantidad)
        {
            try
            {
                return Data.VerificarStock(empresa, almacen, idSucursal, IdProducto, Cantidad);
            }
            catch (Exception ex)
            {
                return ex.Message; ;
        ...
        public List<EVenta> ListaVentaEnvios(string filtro, int Empresa, int sucursal, string FechaIncio, string FechaFin, int numPag, int allReg, int Cant)
        {
            try
            {
                return Data.ListaVentaEnvios( filtro, Empresa,  sucursal,  FechaIncio,  FechaFin,  numPag,  allReg,  Cant);
            }
            catch (Exception ex)
            {
        ...
        public string EstadoEnviado(int IdVenta, int Empresa, string Motivo, int Flag, string Usuario)
        {
            try
            {
                return Data.EstadoEnviado(IdVenta, Empresa, Motivo, Flag, Usuario);
            }
            catch (Exception ex)
            {
